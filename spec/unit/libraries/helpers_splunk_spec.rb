...
describe Server::Helpers::Splunk do
  ...
  it 'return the PATH of the splunk lock file' do
    expect(described_class.splunk_lock_file(node)).to eq File.join(Chef::Config.chef_repo_path, '.chef', 'provisioning-data-chefspec', 'splunk')
  end
  ...
  context 'when splunk attributes are not set' do
    before { node.default['server-provisioning']['splunk'] = nil }
