...
describe Server::Helpers::Builders do
  ...
  context 'when the builder hostname' do
    ...
    context 'is set' do
      before do
        node.default['server-provisioning']['builders']['1']['hostname'] = 'my-cool-build-node-1'
        node.default['server-provisioning']['builders']['2']['hostname'] = 'my-awesome-build-node-2'
        node.default['server-provisioning']['builders']['3']['hostname'] = 'my-great-build-node-3'
      end
  ...
  context 'when the builder specs' do
    ...
    context 'does exist' do
      before do
        node.default['server-provisioning']['builders']['count'] = 3
        node.default['server-provisioning']['builders']['1'] = { 'hostname' => 'my-awesome-build-node-1' }
        node.default['server-provisioning']['builders']['2'] = { 'hostname' => 'my-awesome-build-node-2' }
        node.default['server-provisioning']['builders']['3'] = { 'hostname' => 'my-awesome-build-node-3' }
      end
      ...
      it 'returns the right hostname for the build-nodes' do
        1.upto(node['server-provisioning']['builders']['count']) do |i|
  ...
  context 'when an additional_run_list' do
    ...
    context 'is specified' do
      before do
        node.default['server-provisioning']['builders']['additional_run_list'] = ['recipe[awesome-cookbook]']
        Server::Helpers::Builders.instance_variable_set :@builder_run_list, nil
      end
  ...
  context 'when builders attributes are not set' do
    before { node.default['server-provisioning']['builders'] = nil }
  ...
  describe '#trusted_certs_attributes' do
    ...
    context 'when Supermarket Server is enabled' do
      before do
        allow(Server::Helpers::Supermarket).to receive(:supermarket_enabled?)
          .and_return(true)
        allow_any_instance_of(Chef::ServerAPI).to receive(:get)
      ...
      context 'and the user specify additional certificates' do
        before do
          node.default['server-provisioning']['trusted_certs'] = mock_global_trusted_certs
        end
  ...
  describe '#builders_files' do
    before do
      allow(Server::Helpers).to receive(:provisioning_data_dir).and_return('/chefspec')
  ...
  describe '#builders_attributes' do
    ...
    context 'when delivery-cli attributes are set' do
      before do
        node.default['server-provisioning']['builders']['delivery-cli'] = delivery_actifact
      end
      ...
      context 'and the chefdk_version is set as well' do
        before do
          node.default['server-provisioning']['builders']['chefdk_version'] = chefdk_version
        end
        ...
        context 'plus a bunch of certificates' do
          ...
          context 'plus extra attributes that the user specified' do
            before do
              node.default['server-provisioning']['builders']['attributes'] = extra_builders_attributes
            end
